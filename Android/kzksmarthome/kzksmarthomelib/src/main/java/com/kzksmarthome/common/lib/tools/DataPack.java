...
public class DataPack {
	...
	public static boolean dataFilter(byte[] reMsg) {
		try {
			if (reMsg.length >= 29 && (reMsg[0] & 0xFF) == 0xFF
				...
				Log.d("Checksum", checksum[0] + ";" + reMsg[reMsg.length - 2]);
				Log.d("Checksum", checksum[1] + ";" + reMsg[reMsg.length - 1]);
	...
	public static String[] getAttendBlack(byte[] reMsg) {
		try {
			...
			Log.d("attendBlack", "totalLength--->" + totalLength);
			Log.d("attendBlack", "blacklist length--->" + blackLengh);
			if (totalLength - 5 == blackLengh) {// 判断信息是否"合法"
				...
				if (blackLengh % 8 == 0 && blackLengh != 0) {
					...
					Log.d("attendBlack", "cardsn--->" + cardsn.toString());
					cardsn.deleteCharAt(cardsn.length() - 1);// 删除最后一个"|"
					blackInfo[0] = new String(marker);// 标志位
					blackInfo[1] = cardsn.toString();// 包含卡序号的字符串
					Log.d("attendBlack", "marker bits-->" + blackInfo[0]);
					Log.d("attendBlack", "cardsn-->" + blackInfo[1]);
					return blackInfo;
	...
	public static String[] getDooraccessWhite(byte[] reMsg) {
		try {
			...
			Log.d("doorAccessWhite", "totalLength--->" + totalLength);
			Log.d("doorAccessWhite", "door whitelist len--->" + whiteLengh);
			...
			if (totalLength - 5 == whiteLengh) {// 判断消息是否"合法"
				if (whiteLengh != 0) {
					...
					whiteLog.deleteCharAt(whiteLog.length() - 1);// 删除最后一个"#"
					Log.d("doorAccessWhite", "door no-->" + whiteLog.toString());
					if (whiteLog.length() > 2) {
						whiteInfo[0] = new String(marker);// 标志位
						whiteInfo[1] = whiteLog.toString();// 包含门禁时段信息的字符串
						Log.d("doorAccessWhite", "marker bits-->"
								+ whiteInfo[0]);
						Log.d("doorAccessWhite", "dooraccess whitelist-->"
								+ whiteInfo[1]);
						return whiteInfo;
					} else {
						return null;
					...
					Log.d("doorAccessWhite", "marker bits-->" + whiteInfo[0]);
					Log.d("doorAccessWhite", "dooraccess whitelist-->"
							+ whiteInfo[1]);
					return whiteInfo;
				} else {
	...
	public static String[] getDooraccessTime(byte[] reMsg) {
		try {
			...
			Log.d("doorAccessTime", "totalLength-->" + totalLength);
			Log.d("doorAccessTime", "access times len-->" + timeLengh);
			...
			if (totalLength - 5 == timeLengh) {// 判断数据是否"合法"
				...
				if (timeLengh % 38 == 0 && timeLengh != 0) {
					...
					Log.d("doorAccessTime", "marker bits-->" + timeInfo[0]);
					Log.d("doorAccessTime", "access times-->" + timeInfo[1]);
					return timeInfo;
	...
	public static float getRechargeNum(byte[] reMsg) {
		try {
			...
			System.arraycopy(reMsg, 25, dataLen, 0, 2);// 获得数据总产度
			int totalLength = Tools.byteArrayToShort(dataLen);// 数据总长度
			Log.d("RechargeNum", "totalLength-->" + totalLength);
			if (totalLength == 4) {
				...
				Log.d("RechargeNum", "recharge money-->" + sMoney);
				return sMoney;
	...
	public static float getAllowanceNum(byte[] reMsg) {
		try {
			...
			System.arraycopy(reMsg, 25, dataLen, 0, 2);// 获得数据总产度
			int totalLength = Tools.byteArrayToShort(dataLen);// 数据总长度
			Log.d("AllowanceNum", "totalLength-->" + totalLength);
			if (totalLength == 4) {
				byte[] data = new byte[totalLength];// 信息组
				System.arraycopy(reMsg, 27, data, 0, totalLength);// 获得信息组
				...
				Log.d("AllowanceNum",
						"allowance money-->" + Tools.byteArrayToInt(data)
								/ 100.00f);
				return sMoney;
	...
	public static String[] getCashierWhite(byte[] reMsg) {
		try {
			...
			int totalLength = Tools.byteArrayToShort(dataLen);// 数据总长度
			int whiteLengh = Tools.byteArrayToShort(whiteLen);// 信息组长度
			Log.d("CashierWhite", "totalLength-->" + totalLength);
			Log.d("CashierWhite", "cashier whitelist len-->" + whiteLengh);
			byte[] data = new byte[whiteLengh];// 信息组
			if (totalLength - 5 == whiteLengh) {// 判断是否是"合法"数据
				...
				if (whiteLengh % 8 == 0 && whiteLengh != 0) {
					...
					Log.d("CashierWhite", "marker bits-->" + whiteInfo[0]);
					Log.d("CashierWhite", "cashier whitelist-->" + whiteInfo[1]);
					return whiteInfo;
				} else if (whiteLengh == 0 && "str".equals(new String(marker))) {
					whiteInfo[0] = new String(marker);// 标志位
	...
	public static String[] getPaymentBlack(byte[] reMsg) {
		try {
			...
			int totalLength = Tools.byteArrayToShort(dataLen);// 数据总长度
			int blackLengh = Tools.byteArrayToShort(blackLen);// 信息组长度
			Log.d("PaymentBlack", "totalLength-->" + totalLength);
			Log.d("PaymentBlack", "payment blacklist len-->" + blackLengh);
			byte[] data = new byte[blackLengh];// 信息组
			if (totalLength - 5 == blackLengh) {// 判断是否是"合法"数据
				...
				if (blackLengh % 8 == 0 && blackLengh != 0) {
					...
					blackInfo[0] = new String(marker);// 标志位
					blackInfo[1] = cardsn.toString();// 包含卡序号的字符串
					Log.d("PaymentBlack", "marker bits-->" + blackInfo[0]);
					Log.d("PaymentBlack", "payment blacklist-->" + blackInfo[1]);
					return blackInfo;
				} else if (blackLengh == 0 && "str".equals(new String(marker))) {
					blackInfo[0] = new String(marker);// 标志位
					blackInfo[1] = null;// 包含卡序号的字符串
	...
	public static String getNodeThreshold(byte[] reMsg) {
		try {
			...
			Log.d("NodeThreshold", "totalLength-->" + totalLength);
			Log.d("NodeThreshold", "threshold len-->" + thresholdLength);
			...
			if (totalLength - 2 == thresholdLength) {// 判断是否是"合法"数据
				...
				if (thresholdLength % 5 == 0 && thresholdLength != 0) {
					...
					Log.d("NodeThreshold", "threshold-->" + nodeThreshold);
	...
	public static String[] getCardinfo(byte[] reMsg) {
		try {
			...
			if (reMsg.length > 40) {// 判断是否有数据
				...
				float sMoney = b1.divide(b2)
						.setScale(2, BigDecimal.ROUND_HALF_UP).floatValue();// 获取指令中的金额
	...
	public static byte[] getNodeAddrArray(byte[] reMsg) {
		try {
			...
			Log.d("NodeData", "node num----->" + length);
