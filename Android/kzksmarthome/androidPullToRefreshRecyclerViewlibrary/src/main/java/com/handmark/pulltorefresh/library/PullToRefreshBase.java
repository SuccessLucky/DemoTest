...
import com.handmark.pulltorefresh.library.internal.PrepareRelativeLayout.RefreshPrepareLayoutListener;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public final Mode setCurrentMode(Mode currentMode) {
        return mCurrentMode = currentMode;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        // If we're refreshing, and the flag is set. deliver the event
...
//        }
     ...
     * Used internally for adding view. Need because we override addView to pass-through to the Refreshable View
     */
    public final void addViewInternal(View child, ViewGroup.LayoutParams params) {
     ...
     * @param attrs AttributeSet from wrapped class. Means that anything you include in the XML layout declaration will be routed to the
     *            created View
    ...
    /**
     * Updates the View State when the mode has been set. This does not do any checking that the mode is different to current state so
     * always updates.
     */
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (null != mHeaderLayout) {
            int paddingTop = a.getDimensionPixelOffset(R.styleable.PullToRefresh_ptrHeaderPaddingTop, getResources()
                    .getDimensionPixelOffset(R.dimen.header_footer_top_bottom_padding));
            int paddingBottom = a.getDimensionPixelOffset(R.styleable.PullToRefresh_ptrHeaderPaddingBottom, getResources()
                    .getDimensionPixelOffset(R.dimen.header_footer_top_bottom_padding));
            mHeaderLayout.setInnerLayoutPadding(paddingTop, paddingBottom);
        }
        if (null != mFooterLayout) {
            int paddingTop = a.getDimensionPixelOffset(R.styleable.PullToRefresh_ptrFooterPaddingTop, getResources()
                    .getDimensionPixelOffset(R.dimen.header_footer_top_bottom_padding));
            int paddingBottom = a.getDimensionPixelOffset(R.styleable.PullToRefresh_ptrFooterPaddingBottom, getResources()
                    .getDimensionPixelOffset(R.dimen.header_footer_top_bottom_padding));
            mFooterLayout.setInnerLayoutPadding(paddingTop, paddingBottom);
        }
        a.recycle();
    ...
    public static enum AnimationStyle {
        ...
        /**
         * Maps an int to a specific mode. This is needed when saving state, or inflating the view from XML where the mode is given through
         * a attr int.
         * 
    ...
    public static enum Mode {
        ...
        /**
         * Only allow the user to Pull from the end of the Refreshable View to refresh. The start is either the Bottom or Right, depending
    ...
    public static interface OnPullEventListener<V extends View> {
         ...
         * @param direction - One of {@link Mode#PULL_FROM_START} or {@link Mode#PULL_FROM_END} depending on which direction the user is
         *            pulling. Only useful when <var>state</var> is {@link State#PULL_TO_REFRESH} or {@link State#RELEASE_TO_REFRESH}.
         */
    ...
    final class SmoothScrollRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mStartTime == -1) {
                ...
                /**
                 * We do do all calculations in long to reduce software float calculations. We use 1000 as it gives us good accuracy and
    ...
    public void setRefreshPrepareLayoutListener(RefreshPrepareLayoutListener refreshPrepareLayoutListener) {
        if (null != mHeaderLayout) {
            mHeaderLayout.setRefreshPrepareLayoutListener(refreshPrepareLayoutListener);
        }
    }

    public void hasMore(boolean hasMore) {
        if (null != mFooterLayout) {
            mFooterLayout.setHasMore(hasMore);
        }
    }
}
