 ...
 * @author laixj
 * @version V1.0
 * @Title: SceneRightsSelectListAdapter
...
public class SceneRightsSelectListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public SceneRightsSelectListAdapter(Context context, List<SceneInfo> sceneList) {
        this(context);
        this.sceneList = sceneList;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int position) {
        ...
        if (null != sceneInfo) {
            ...
            if (sceneInfo.isSelected()) {
                sceneViewHolder.smartSceneRightsListSelectIv.setVisibility(View.VISIBLE);
            } else {
                sceneViewHolder.smartSceneRightsListSelectIv.setVisibility(View.INVISIBLE);
            }
            Glide
                    .with(mContext)
            ...
            //sceneViewHolder.smartSceneRightsListIconIv.setImageResource(sceneInfo.getId());
        }
    }
    ...
    class SceneViewHolder extends RecyclerView.ViewHolder {
        ...
        public void onClick() {
            SceneInfo clicked = sceneList.get(getPosition());
            if (null == clicked) {
                return;
            }
            clicked.setSelected(!clicked.isSelected());
            sceneList.set(getPosition(), clicked);
            /*if (clicked.isSelected()) {
                smartSceneRightsListSelectIv.setVisibility(View.VISIBLE);
            } else {
                smartSceneRightsListSelectIv.setVisibility(View.INVISIBLE);
