...
- (void)setup{
    ...
    /** 浮标image*/
    _imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 11,28,34)];
    _imageView.image = [UIImage imageNamed:@"亮度显示_"];
    [_scrollShowTextView addSubview:_imageView];
    
    /** 浮标数值显示label*/
    _scrollShowTextLabel = [[UILabel alloc]initWithFrame:CGRectMake(-4, 9 , 36, 36)];
    ...
    UIPanGestureRecognizer *panGR = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(longGRAction:)];
...
- (void)longGRAction:(UILongPressGestureRecognizer *)recognizer{
    ...
    if(recognizer.state == UIGestureRecognizerStateEnded){
        ...
        if((translation.x >=20 && ((_hyMaxValue/self.frame.size.width) * translation.x) <= _hyMaxValue)){
