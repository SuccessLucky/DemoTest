...
- (void)commonInit{
    ...
    _labelOnThumb.textColor = [UIColor blueColor];
    _labelOnThumb.font = [UIFont systemFontOfSize:10];
    
    [self addSubview:_labelOnThumb];
    
    _labelAboveThumb = [[UILabel alloc] init];
    _labelAboveThumb.backgroundColor = [UIColor blueColor];
    ...
    _labelAboveThumb.font = [UIFont systemFontOfSize:10];
...
- (void)layoutSubviews{
    ...
    if (!_directionType) {
        thumbHeight = 98.f *  _backgroundImageView.bounds.size.height / 16.f;   // thumb height is relative to track height
        thumbWidth = 91.f * thumbHeight / 98.f; // thumb width and height keep the same ratio as the original image size
    }else{
        thumbWidth = 98.f *  _backgroundImageView.bounds.size.width / 16.f;
    ...
    _labelOnThumb.frame = CGRectMake(_thumbImageView.frame.origin.x, _thumbImageView.frame.origin.y, _thumbImageView.frame.size.width, _thumbImageView.frame.size.height);
    if (!_directionType) {
